...
这种在 Core Foundation 框架和 Foundation 框架之间交换使用数据类型的技术就叫 Toll-Free Bridging。
...
//这时候，即使开启ARC，也需要手动执行CFRelease，因为此时对象管理已经修改CF管理了
CFRelease(aCFString);
```

**附录：**
并不是所有的 CF 对象都支持 Toll-Free Bridging！以下是支持该技术的类型表。
