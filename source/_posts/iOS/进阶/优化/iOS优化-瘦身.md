...
![download_install_size](../../../../img/iOS/进阶/优化/download_install_size.png)
...
我们在下载安装包时刻，不会下载 OnDemandResources 文件夹中的资源，起到减小下载安装包尺寸的目的。

**结论：`Enable On Demand Resources` 是默认设置为 YES 的，可根据项目需要决定将哪些资源设置 Tag；**
...
**结论：利用工具粗检测一下哪些图片没有被使用，考虑到工具原理的不准确性，所以需要删除之前需要人工确定图片确实没有被引用了。对于工具无法检测出来的图片，就只能人工进行筛查了，可每人分配几个模块，提高效率。**
...
- [ImageOptim](https://github.com/ImageOptim/ImageOptim)： 客户端工具，支持无损压缩及有损压缩两种形式；
...
**结论：能用 Asset Catalogs 管理的资源，尽量使用 Asset Catalogs 来管理。使用 Asset Catalog 管理图片不要对图片进行无损压缩，最终起不到压缩效果，如果想要压缩，可以采用上面所提到的有损压缩方式。**
...
这种方式的性价比很高，改动一项配置，就可能会带来收益，但是有具有一定的风险，需要谨慎。
...
- [今日头条 iOS 安装包瘦身-3](https://mp.weixin.qq.com/s/TnqAqpmuXsGFfpcSUqZ9GQ)
