...
![下载及安装大小示意图](../../../../img/iOS/进阶/优化/瘦身/download_install_size.png)
...
**其本质是使正常的 png 图片变成了一个优化后的 CgBI 格式的 png**。可以利用[pngcheck](http://www.libpng.org/pub/png/apps/pngcheck.html)查看处理前的图片信息，利用[pngdefry](http://www.jongware.com/pngdefry.html)查看处理后的图片信息（其还可以将 CgBI 格式的 png还原回去，这个功能跟pngcrush工具解码功能类似）。
...
> 通过对 Mach-O 文件的了解，可以知道 `__TEXT:__objc_methname:` 中包含了代码中的所有方法，而`__DATA__objc_selrefs`中则包含了所有被使用的方法的引用，通过取两个集合的差集就可以得到所有未被使用的代码。具体可看[objc_cover](https://github.com/nst/objc_cover)
