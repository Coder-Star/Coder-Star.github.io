...
> Tips: Xcode12 打出的模拟器版的 framework 会包括 x86_64、arm64 两种架构，但是这里面的 arm64 并不能代替真机的 arm64，所以需要对此架构进行擦除或者在打包时进行排除，排除方式为将 Build Setting 的 Excluded Architectures 下的 Any iOS Simulator SDK 中加上 arm64

## 四、打包（利用 cocoapods-packager 插件）
