...
let修饰的属性只可以被赋值一次，如果在方法中，除了使用这种形式外，还可以使用`let number:Int;number = 1`这种形式，不过两种形式赋值一次之后都不可以再次赋值了。
...
* **static或class修饰属性时，属性会自动进行懒加载，并且是线程安全的，只有第一次去访问它的时候才会进行赋值。其线程安全的实现是通过在dispatch_once内部进行赋值，保证初始化操作只执行一次**
...
* 类型属性：被static、class修饰的属性；可以用在枚举、类、结构体等中；
