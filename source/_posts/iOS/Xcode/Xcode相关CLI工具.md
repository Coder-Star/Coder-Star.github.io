...
我们还可以在[SwiftDemangle.h](https://github.com/apple/swift/blob/main/include/swift/SwiftDemangle/SwiftDemangle.h)看到swift底层该函数名称 -- `swift_demangle_getDemangledName`，项目中需要加入`libswiftDemangle.tbd`。
...
为什么要把这两个工具放到一起说呢？因为这两个工具之间有一定的关系。

#### otool

作用： 针对目标文件的展示工具，用来发现应用中使用到了哪些系统库，调用了其中哪些方法，使用了库中哪些对象及属性。
otool 路径：`/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/otool`
objdump 路径：`/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/objdump`
...
> 如果安装了`MonkeyDev`，内置了`class-dump`
